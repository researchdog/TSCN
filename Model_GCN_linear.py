...
class GCN_linear(nn.Module):
    ...
    def __init__(self):
        ...
        self.gcn1=GCN(self.input_size,20)
        self.gcn2=GCN(20,30)
        self.LSTM = nn.LSTM(input_size=4, hidden_size=10,num_layers=1)
        
        self.liner1 = nn.Linear(72000, 120)
    ...
    def forward(self,adjacency,h):
        ...
        #h=F.relu(self.gcn4(adjacency,h,self.seq_len))
        #print(h.shape,'h.shape')
        #=h.reshape(self.num_nodes,self.seq_len,batch_size,h.shape[2])
        #print('h',h.shape)
        #hide=torch.zeros([batch_size,10])
        #for i,h_L in enumerate(h):
            #lstm_out,(h_n,h_c)=self.LSTM(h_L,None)
            #print(lstm_out[:,-1,:].shape,'lstm_out[:,-1,:].shape')
            #print(lstm_out.shape,'lstm.shape')
            #print(lstm_out[:,-1,:].shape,hide.shape,lstm_out.shape,'hide.shape')
            #hide = torch.add(lstm_out[-1,:,:],hide)
        #h=hide
        #print('h_relu**********************',h.shape)
        #print(h.shape,'h.shape')
        #h=h.reshape(batch_size,self.seq_len)
        #print(h.shape,'shape')
        h=h.view(-1, h.shape[0]*h.shape[1]*h.shape[2])
        h=F.relu(self.liner1(h))
        h=F.relu(self.liner2(h))
        h=F.relu(self.liner3(h))
        #h=self.dropout(h)
        h=F.sigmoid(h)
        return h
